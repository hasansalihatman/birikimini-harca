...
export default function App() {
  ...
  const removeToCart = (item) => {
    ...
    copyOfCart.splice(
      copyOfCart.findIndex((e) => e.id === item.id),
      1
    );
    setCart(copyOfCart);
    setTotalSpendings((prev) => Number(prev) - Number(item.price));
      ...
      <>
        {summary.map((item, index) => (
          <tr key={index}>
            ...
            <td>
              <img style={{backgroundSize:'cover'}} src={item.img} width={50} height={50} alt="" />
            </td>
            <td>{item.title}</td>
            <td>₺{item.price}</td>
            <td>{item.amount}</td>
    ...
    <div>
      ...
      >
        ...
        {stage === 2 && (
          <Button
            onClick={() => setStage(3)}
            disabled={totalSpendings > 0 ? false : true}
            color="primary"
          >
            Harcama Özeti
          </Button>
      ...
      {stage === 1 ? (
        <Container>
          <Card style={{ padding: "24px", margin: "12px", marginTop: "64px" }}>
            <Form>
              <FormGroup>
                <Label for="savings">Aylık Biriktirdiğiniz Miktar</Label>
                <Input
                  id="savings"
                  name="savings"
                  type="number"
              ...
              <Button
                ...
                disabled={savings && months ? false : true}
                onClick={() => {
                  calculateSavings();
                }}
                color="primary"
        ...
        <>
          <Container>
            <Card
              style={{ padding: "24px", margin: "12px", marginTop: "64px" }}
            >
              <Row>
                <Col
                  style={{
                ...
                >
                  <CardTitle style={{ color: "#6c757d" }} tag={"h5"}>
                    Birikim
                  </CardTitle>
                  <CardText tag={"h1"}>₺{totalSavings}</CardText>
                </Col>
                <Col
                  style={{
                    display: "flex",
                ...
                >
                  ...
                  <CardText tag={"h1"}>₺{totalSpendings}</CardText>
                </Col>
                <Col
                  style={{
                    display: "flex",
        ...
        <Container >
          <Card className="forPhones" style={{ padding: "24px", margin: "12px", marginTop: "64px", border:'none'}}>
            <Row>
              <Table striped>
                <thead>
                  <tr>
